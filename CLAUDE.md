...
| 11 | PT Community — 익명 케이스 토론 | Red Flag 트리거 케이스 공유 피드, 멤버 전용 비공개 커뮤니티 |
...
### Phase 11 상세 — PT Community (익명 케이스 토론)

**목적**: Reddit에서 자생적으로 일어나는 PT 케이스 공유 행동을 Sage Pontus 서비스 내부로 흡수 → 멤버 전용 임상 토론 커뮤니티

**핵심 차별화**: Red Flag 알람이 실제로 트리거된 케이스만 공유 가능 → 임상적으로 검증된 케이스 피드, Reddit 대비 신호/잡음 비율 압도적 우위

| 작업 | 설명 |
|------|------|
| 케이스 공유 플로우 | 분석 결과 화면 → "Share anonymously" 버튼 → 환자 식별정보 자동 제거 후 피드에 게시 |
| 커뮤니티 피드 | 최신/인기 케이스 목록, 조건별 필터(Red Flag 종류, 신체 부위) |
| 댓글 & 투표 | 멤버 간 임상 의견 교환, "나도 비슷한 케이스 경험" 투표 |
| 멤버 전용 접근 | 유료 구독 멤버만 읽기/쓰기 — 커뮤니티가 유료 전환 동기로 작동 |
| HIPAA 가드 | 공유 전 PHI 자동 스캔 + 사용자 확인 단계 의무화 |

**출시 타이밍**: 유료 고객 50명 이상 확보 후 비공개 베타 → 활성화 확인 후 전체 오픈

**로드맵 연결**: Phase 7 flywheel 데이터(임상가 교정 행위) + Phase 10 클리닉 통계가 커뮤니티 케이스와 결합 → 집단 지성 기반 알람 weight 갱신
